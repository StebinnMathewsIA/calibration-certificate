...
export default function IssuedScreen() {
  ...
  // Content preview rendered from the same form data as the signed PDF
  // (Android WebView cannot display local PDFs; the shareable file is
  // always the signed PDF itself).
  const previewHtml = useMemo(() => {
    if (!form || !validateReadyToSign(form).ready) return null;
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <SectionCard title="Verification details">
        <Pressable onPress={() => setShowVerification((v) => !v)} hitSlop={8}>
