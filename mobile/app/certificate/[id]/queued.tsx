...
/** Order-tracking view of the sign queue for one certificate. */
    ...
    <ScrollView style={styles.screen} contentContainerStyle={{ paddingBottom: 40 }}>
      <SectionCard title={`Certificate ${record.certificateNumber ?? ''}`}>
        {STEPS.map((step, i) => {
          ...
          const color = reached || (active && done) ? colors.green : active ? colors.amber : colors.muted;
          return (
            <View key={step.label} style={{ flexDirection: 'row', marginBottom: 12 }}>
              <View style={{ width: 28, alignItems: 'center' }}>
                {active && !done ? (
                  ...
                  <Text style={{ fontSize: 16, color }}>{reached || (active && done) ? '✓' : '○'}</Text>
                )}
              </View>
              <View style={{ flex: 1 }}>
      ...
      <View style={{ marginHorizontal: 12 }}>
        {done ? (
          <Button
            ...
            onPress={() => router.replace({ pathname: '/certificate/[id]/issued', params: { id } })}
          />
        ) : null}
        <Button title="Back to calibrations" kind="secondary" onPress={() => router.replace('/home')} />
      </View>
    </ScrollView>
  );
}
