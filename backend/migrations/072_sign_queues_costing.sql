-- Job card sign queues its costing for real (#110, #99).
--
-- It used to queue `blocked` with "ImportWorkTaskSpares builder not
-- implemented yet", then "no way to learn the task". Both are resolved: the
-- builder exists and tasks are mirrored per work order. What remains is to
-- resolve the task at sign time and queue pending when we have one.
--
-- Still blocked, deliberately, when the task is unknown. A costing line
-- with no parent task is not something to guess at: OnKey hangs every
-- spare off a task, and inventing one would put a charge on the wrong job.
-- Blocked means WE declined; the data is held and one flip releases it.
--
-- Idempotent.
