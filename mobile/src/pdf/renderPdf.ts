 ...
 * (queue integrity + upload verification). */
export async function renderCertificatePdf(form: CalibrationForm): Promise<RenderedPdf> {
  const { uri } = await Print.printToFileAsync({ html: certificateHtml(form) });
