...
/** Backfill a reserved number into the record AND the verification payload.
 * No-op if the record already has a number (numbers are immutable). */
export function assignCertificateNumber(id: string, certificateNumber: string): void {
  const rec = getById(id);
  if (!rec || rec.certificateNumber) return;
  const form = { ...rec.form, certificateNumber };
  db.runSync(
